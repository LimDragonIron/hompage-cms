import { useState } from "react";
import { useContentDraftEditor } from "@/hooks/useContentDraftEditor";
import {
  createOrFetchDraftNews,
  updateDraftNews,
  completeDraftNews,
  deleteDraftNews,
  uploadImage
} from "@/api";
import { Button } from "@/components/ui/button";
import { TiptapEditor } from "@/components/TiptapEditor";
import { isContentEmpty, isValidTag } from "@/lib/contentUtils";

export function NewsCreatePage() {
  const [tagInput, setTagInput] = useState("");
  const [isComposing, setIsComposing] = useState(false);

  const {
    title, setTitle,
    content, setContent,
    hashtags, setHashtags,
    fileId,
    imagePreview,
    currentImageUrl,
    uploading, draftId, loadingDraft,
    handleFileChange, handleComplete, handleCancel, autoSave,
  } = useContentDraftEditor({
    queryKey: "news-draft",
    createOrFetchDraft: createOrFetchDraftNews,
    updateDraft: updateDraftNews,
    completeDraft: completeDraftNews,
    deleteDraft: deleteDraftNews,
    uploadImageApi: (file: File, context?: any) => uploadImage(file, context),
    afterCompleteNavigate: "/news",
    afterCancelNavigate: "/news",
    initialState: {},
    useTag: true,
    usePlatformLinks: false,
  });

  // 태그 추가/삭제
  const handleAddTag = () => {
    const newTag = tagInput.trim();
    if (
      !newTag ||
      hashtags.includes(newTag) ||
      !isValidTag(newTag)
    ) {
      alert("유효하지 않거나 중복된 태그입니다. (한글, 영문, 숫자만 허용)");
      return;
    }
    const newTags = [...hashtags, newTag];
    setHashtags(newTags);
    setTagInput("");
    setTimeout(() => {
      autoSave({ hashtags: newTags });
    }, 0);
  };

  const handleRemoveTag = (tag: string) => {
    const newTags = hashtags.filter((t) => t !== tag);
    setHashtags(newTags);
    setTimeout(() => {
      autoSave({ hashtags: newTags });
    }, 0);
  };

  // 정식 등록시 검증 함수
  const handleValidate = () => {
    if (!fileId) {
      return "이미지를 등록해야 정식 등록이 가능합니다.";
    }
    if (!title.trim()) {
      return "제목을 입력해야 정식 등록이 가능합니다.";
    }
    if (isContentEmpty(content)) {
      return "내용을 입력해야 정식 등록이 가능합니다.";
    }
    if (hashtags.length === 0) {
      return "태그를 하나 이상 입력해야 합니다.";
    }
    return undefined;
  };

  if (loadingDraft || draftId == null) {
    return <div className="p-6 sm:p-8 text-center">임시 저장 불러오는 중...</div>;
  }

  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-full sm:max-w-lg md:max-w-xl mx-auto">
      <h2 className="text-lg sm:text-xl font-bold mb-4">뉴스 생성</h2>
      <div className="mb-4">
        <label className="block mb-2 font-semibold text-sm sm:text-base">제목</label>
        <input
          className="border rounded px-3 py-2 w-full text-sm sm:text-base focus:outline-blue-400"
          value={title}
          onChange={e => {
            setTitle(e.target.value);
            autoSave({ hashtags });
          }}
          disabled={uploading}
          maxLength={100}
        />
      </div>
      <div className="mb-4">
        <label className="block mb-2 font-semibold text-sm sm:text-base">태그</label>
        <div className="flex gap-2 mb-2 flex-wrap">
          {hashtags.map((tag) => (
            <span
              key={tag}
              className="bg-blue-100 text-blue-600 rounded px-2 py-1 text-xs flex items-center gap-1"
            >
              #{tag}
              <button
                type="button"
                onClick={() => handleRemoveTag(tag)}
                className="ml-1 text-red-400 hover:text-red-700"
              >
                ×
              </button>
            </span>
          ))}
        </div>
        <div className="flex gap-2">
          <input
            className="border rounded px-2 py-1 w-40"
            value={tagInput}
            onChange={(e) => setTagInput(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter" && !isComposing) {
                e.preventDefault();
                handleAddTag();
              }
            }}
            onCompositionStart={() => setIsComposing(true)}
            onCompositionEnd={() => setIsComposing(false)}
            disabled={uploading}
            placeholder="태그 추가 후 Enter"
          />
          <Button size="sm" onClick={handleAddTag} disabled={uploading}>
            추가
          </Button>
        </div>
        <div className="text-xs text-gray-500 mt-1">
          한글, 영문, 숫자만 허용, 중복/공백/특수문자 불가
        </div>
      </div>
      <div className="mb-4">
        <label className="block mb-2 font-semibold text-sm sm:text-base">이미지</label>
        {currentImageUrl && !imagePreview && (
          <div className="mb-2">
            <img src={currentImageUrl} alt="현재 이미지" className="w-full max-w-xs sm:max-w-sm max-h-40 sm:max-h-56 rounded object-cover" />
          </div>
        )}
        {imagePreview && (
          <div className="mb-2">
            <img src={imagePreview} alt="새 이미지 미리보기" className="w-full max-w-xs sm:max-w-sm max-h-40 sm:max-h-56 rounded object-cover" />
          </div>
        )}
        <input
          type="file"
          accept="image/png, image/jpeg, image/webp"
          onChange={e => handleFileChange(e, { contentType: "NEWS" })}
          disabled={uploading}
          className="block w-full text-sm"
        />
        {uploading && <div className="text-gray-500 mt-1">이미지 업로드 중...</div>}
      </div>
      <div className="mb-4">
        <label className="block mb-2 font-semibold text-sm sm:text-base">내용</label>
        <div className="border rounded bg-white min-h-[200px]">
          <TiptapEditor
            content={content}
            onChange={val => {
              setContent(val);
              autoSave({ hashtags });
            }}
            editable={!uploading}
          />
        </div>
      </div>
      <div className="flex flex-col gap-2 sm:flex-row sm:gap-3 mt-6">
        <Button
          variant="secondary"
          onClick={handleCancel}
          className="w-full sm:w-auto"
        >
          취소
        </Button>
        <Button
          variant="default"
          onClick={() => handleComplete(handleValidate)}
          disabled={
            uploading ||
            !fileId ||
            !title.trim() ||
            isContentEmpty(content) ||
            hashtags.length === 0
          }
          className="w-full sm:w-auto"
        >
          등록 완료
        </Button>
      </div>
    </div>
  );
}